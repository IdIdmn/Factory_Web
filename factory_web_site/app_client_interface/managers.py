...
class OrderQuerySet(models.QuerySet): 
    ...
    def find_by_order_type(self, order_type, can_include=False):
        if can_include:
            return self.annotate(lower_order_type=Lower('order_type')).filter(lower_order_type__icontains=order_type.lower())
        else:
